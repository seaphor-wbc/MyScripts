...
function patch_list
{
yum check-update > tmp.txt 
if [ "`grep $ARCH tmp.txt`" == "" ]; then
...
function part_tmp
{
grep "[[:space:]]/tmp[[:space:]]" /etc/fstab > tmp.txt
if [ "`grep 'tmp' tmp.txt`" == "" ]; then
...
function nodev_tmp
{
grep "[[:space:]]/tmp[[:space:]]" /etc/fstab | grep nodev > tmp.txt
mount | grep "[[:space:]]/tmp[[:space:]]" | grep nodev >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function nosuid_tmp
{
grep "[[:space:]]/tmp[[:space:]]" /etc/fstab | grep nosuid > tmp.txt
mount | grep "[[:space:]]/tmp[[:space:]]" | grep nosuid >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function noexec_tmp
{
grep "[[:space:]]/tmp[[:space:]]" /etc/fstab | grep noexec > tmp.txt
mount | grep "[[:space:]]/tmp[[:space:]]" | grep noexec >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function part_var
{
grep "[[:space:]]/var[[:space:]]" /etc/fstab > tmp.txt
if [ "`grep 'var' tmp.txt`" == "" ]; then
...
{
grep -e "^/tmp[[:space:]]" /etc/fstab | grep /var/tmp > tmp.txt
mount | grep -e "^/tmp[[:space:]]" | grep /var/tmp >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
grep "[[:space:]]/var/log[[:space:]]" /etc/fstab > tmp.txt
if [ "`grep 'var' tmp.txt`" == "" ]; then
...
{
grep "[[:space:]]/var/log/audit[[:space:]]" /etc/fstab > tmp.txt
if [ "`grep 'var' tmp.txt`" == "" ]; then
...
function part_home
{
grep "[[:space:]]/home[[:space:]]" /etc/fstab > tmp.txt
if [ "`grep 'home' tmp.txt`" == "" ]; then
...
{
grep "[[:space:]]/home[[:space:]]" /etc/fstab > tmp.txt
mount | grep /home > tmp.txt
if [ "`grep nodev tmp.txt`" == "" ]; then
...
{
grep /dev/shm /etc/fstab | grep nodev > tmp.txt
mount | grep /dev/shm | grep nodev >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
grep /dev/shm /etc/fstab | grep nosuid > tmp.txt
mount | grep /dev/shm | grep nosuid >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
grep /dev/shm /etc/fstab | grep noexec > tmp.txt
mount | grep /dev/shm | grep noexec >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function sticky_set
{
df --local -P | awk {'if (NR!=1) print $6'} | xargs -I '{}' find '{}' -xdev -type d \( -perm -0002 -a ! -perm -1000 \) 2>/dev/null > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
grep gpgcheck /etc/yum.conf > tmp.txt
if [ "`grep 1 tmp.txt`" == "" ]; then
...
function grub_owner
{
stat -L -c "%u %g" /boot/grub2/grub.cfg | egrep "0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function grub_perms
{
stat -L -c "%a" /boot/grub2/grub.cfg | egrep ".00" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function boot_pass
{
grep "^set superusers" /boot/grub2/grub.cfg > tmp.txt
grep "^password" /boot/grub2/grub.cfg >> tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function core_dumps
{
grep "hard core" /etc/security/limits.conf > tmp.txt
sysctl fs.suid_dumpable > tmp2.txt
if [ "`cat tmp.txt`" == "" ]; then
	PASSA="Fail"
else
	PASSA="Pass"
fi
if [ "`grep 0 tmp2.txt`" == "" ]; then
	PASSB="Fail"
else
	PASSB="Pass"
fi
...
{
sysctl kernel.randomize_va_space > tmp.txt
if [ "`grep 2 tmp.txt`" == "" ]; then
...
{
rpm -q telnet-server > tmp.txt
if [ "`grep 'not installed' tmp.txt`" == "" ]; then
...
{
chkconfig --list chargen-dgram > tmp.txt 2>&1
if [ "`grep -i error tmp.txt`" == "" ]; then
...
{
chkconfig --list chargen-stream > tmp.txt 2>&1
if [ "`grep -i error tmp.txt`" == "" ]; then
...
{
chkconfig --list daytime-dgram > tmp.txt 2>&1
if [ "`grep -i error tmp.txt`" == "" ]; then
...
{
chkconfig --list daytime-stream > tmp.txt 2>&1
if [ "`grep -i error tmp.txt`" == "" ]; then
...
function echo_dgram
{
chkconfig --list echo-dgram > tmp.txt 2>&1
if [ "`grep -i error tmp.txt`" == "" ]; then
...
{
chkconfig --list echo-stream > tmp.txt 2>&1
if [ "`grep -i error tmp.txt`" == "" ]; then
...
{
chkconfig --list tcpmux-server > tmp.txt 2>&1
if [ "`grep -i error tmp.txt`" == "" ]; then
...
function daem_umask
{
grep umask /etc/sysconfig/init > tmp.txt
if [ "`grep '027' tmp.txt`" == "" ]; then
...
function check_xwin
{
ls -l /etc/systemd/system/default.target | grep graphical.target > tmp.txt
if [ "`cat tmp.txt`" != "" ]; then
	PASSC="Fail"
else
	PASSC="Pass"
fi
rpm -q xorg-x11-server-common > tmp2.txt
if [ "`grep 'not installed' tmp2.txt`" == "" ]; then
	PASSD="Fail"
else
	PASSD="Pass"
fi
...
{
systemctl is-enabled avahi-daemon > tmp.txt
if [ "`grep 'enabled' tmp.txt`" == "" ]; then
...
function check_ntp
{
grep "restrict default" /etc/ntp.conf > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
	PASSE="Fail"
else
	if [ "`grep 'restrict default kod nomodify notrap nopeer noquery' tmp.txt`" == "" ]; then
		PASSE="Fail"
	else
		PASSE="Pass"
	fi
...
grep "^server" /etc/ntp.conf > tmp3.txt
if [ "`grep 'time01' tmp3.txt`" == "" ]; then
	PASSG="Fail"
else
	PASSG="Pass"
fi
grep "ntp:ntp" /etc/sysconfig/ntpd > tmp4.txt
if [ "`cat tmp4.txt`" == "" ]; then
	PASSH="Fail"
else
	PASSH="Pass"
fi
...
function local_mail
{
netstat -an | grep LIST | grep ":25[[:space:]]" > tmp.txt
if [ "`grep 'tcp' tmp.txt | grep 127.0.0.1:25 | grep LISTEN`" == "" ]; then
...
function forward_ip
{
/sbin/sysctl net.ipv4.ip_forward > tmp.txt
if [ "`grep '0' tmp.txt`" == "" ]; then
...
function packet_snd
{
/sbin/sysctl net.ipv4.conf.all.send_redirects > tmp.txt
/sbin/sysctl net.ipv4.conf.default.send_redirects > tmp2.txt
if [ "`grep '0' tmp.txt`" == "" ]; then
	PASSI="Fail"
else
	PASSI="Pass"
...
function packet_src
{
/sbin/sysctl net.ipv4.conf.all.accept_source_route > tmp.txt
/sbin/sysctl net.ipv4.conf.default.accept_source_route > tmp2.txt
if [ "`grep '0' tmp.txt`" == "" ]; then
	PASSK="Fail"
else
	PASSK="Pass"
...
function icmp_redir
{
/sbin/sysctl net.ipv4.conf.all.accept_redirects > tmp.txt
/sbin/sysctl net.ipv4.conf.default.accept_redirects > tmp2.txt
if [ "`grep '0' tmp.txt`" == "" ]; then
	PASSM="Fail"
else
	PASSM="Pass"
...
function susp_pkts
{
/sbin/sysctl net.ipv4.conf.all.log_martians > tmp.txt
/sbin/sysctl net.ipv4.conf.default.log_martians > tmp2.txt
if [ "`grep '1' tmp.txt`" == "" ]; then
	PASSO="Fail"
else
	PASSO="Pass"
fi
if [ "`grep '1' tmp2.txt`" == "" ]; then
	PASSP="Fail"
else
	PASSP="Pass"
fi
...
function ignor_bcst
{
/sbin/sysctl net.ipv4.icmp_echo_ignore_broadcasts > tmp.txt
if [ "`grep '1' tmp.txt`" == "" ]; then
...
function bad_error
{
/sbin/sysctl net.ipv4.icmp_ignore_bogus_error_responses > tmp.txt
if [ "`grep '1' tmp.txt`" == "" ]; then
...
function tcp_svn
{
/sbin/sysctl net.ipv4.tcp_syncookies > tmp.txt
if [ "`grep '1' tmp.txt`" == "" ]; then
...
function host_allow
{
/bin/ls -l /etc/hosts.allow > tmp.txt
if [ "`grep 'root root' tmp.txt`" == "" ]; then
...
function host_deny
{
/bin/ls -l /etc/hosts.deny > tmp.txt
if [ "`grep 'root root' tmp.txt`" == "" ]; then
...
function enable_fwd
{
systemctl is-enabled firewalld > tmp.txt
if [ "`grep 'enabled' tmp.txt`" == "" ]; then
...
function syslog_act
{
systemctl is-enabled rsyslog > tmp.txt
if [ "`grep 'enabled' tmp.txt`" == "" ]; then
...
do
stat -L -c "%a" $i | grep ".00" > tmp.txt
stat -L -c "%u %g" $i | egrep "0 0" > tmp2.txt
if [ "`cat tmp.txt`" == "" ]; then
	PASSQ="Fail"
else
	PASSQ="Pass"
...
{
grep "^*.*[^I][^I]*@" /etc/rsyslog.conf > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
systemctl is-enabled crond > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/anacrontab | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function cron_perms
{
stat -L -c "%a %u %g" /etc/crontab | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/cron.hourly | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/cron.daily | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/cron.weekly | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/cron.monthly | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/cron.d | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
if [ -a /etc/at.allow ]; then
	stat -L -c "%a %u %g" /etc/at.allow | egrep ".00 0 0" > tmp2.txt
	if [ "`cat tmp2.txt`" == "" ]; then
		PASST="Fail"
	else
		PASST="Pass"
...
function loglvl_ssh
{
grep "^LogLevel" /etc/ssh/sshd_config > tmp.txt
if [ "`grep INFO tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/ssh/sshd_config | egrep ".00 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function x11fwd_ssh
{
grep "^X11Forwarding" /etc/ssh/sshd_config > tmp.txt
if [ "`grep -i no tmp.txt`" == "" ]; then
...
{
grep "^MaxAuthTries" /etc/ssh/sshd_config > tmp.txt
if [ "`cat tmp.txt | awk '{print $2}'`" -ge "6" ]; then
...
{
grep "^IgnoreRhosts" /etc/ssh/sshd_config > tmp.txt
if [ "`grep -i yes tmp.txt`" == "" ]; then
...
{
grep "^HostbasedAuthentication" /etc/ssh/sshd_config > tmp.txt
if [ "`grep -i no tmp.txt`" == "" ]; then
...
{
grep "^PermitRootLogin" /etc/ssh/sshd_config | awk '{print $2}' > tmp.txt
if [ "`cat tmp.txt`" == "yes" ]; then
...
{
grep "^PermitEmptyPasswords" /etc/ssh/sshd_config > tmp.txt
if [ "`grep -i no tmp.txt`" == "" ]; then
...
{
grep PermitUserEnvironment /etc/ssh/sshd_config > tmp.txt
if [ "`grep -i no tmp.txt`" == "" ]; then
...
{
grep "Ciphers" /etc/ssh/sshd_config > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function alive_ssh
{
grep "^ClientAliveInterval" /etc/ssh/sshd_config > tmp.txt
grep "^ClientAliveCountMax" /etc/ssh/sshd_config > tmp2.txt
if [ "`cat tmp.txt | awk '{print $2}'`" != "" ]; then
	if [ "`cat tmp.txt | awk '{print $2}'`" -ge "301" ]; then
		PASSX="Pass"
	else
		PASSX="Fail"
	fi
...
if [ "`cat tmp2.txt | awk '{print $2}'`" != "" ]; then
	if [ "`cat tmp2.txt | awk '{print $2}'`" == "0" ]; then
		PASSY="Pass"
	else
		PASSY="Fail"
	fi
...
{
grep "^AllowUsers" /etc/ssh/sshd_config > tmp.txt
grep "^AllowGroups" /etc/ssh/sshd_config > tmp2.txt
if [ "`cat tmp.txt`" == "" ] && [ "`cat tmp2.txt`" == "" ]; then
        PASSZ="Fail"
fi
if [ "`cat tmp.txt`" == "" ] && [ "`cat tmp2.txt`" != "" ]; then
        PASSZ="Pass-G"
...
grep "^DenyUsers" /etc/ssh/sshd_config > tmp3.txt
grep "^DenyGroups" /etc/ssh/sshd_config > tmp4.txt
if [ "`cat tmp3.txt`" == "" ] && [ "`cat tmp4.txt`" == "" ]; then
        PASSAA="Fail"
fi
if [ "`cat tmp3.txt`" == "" ] && [ "`cat tmp4.txt`" != "" ]; then
        PASSAA="Pass-G"
...
function banner_ssh
{
grep "^Banner" /etc/ssh/sshd_config > tmp.txt
if [ "`cat tmp.txt | awk '{print $2}'`" == "" ]; then
...
{
authconfig --test | grep hashing | grep sha512 > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
grep minlen /etc/security/pwquality.conf | sed -e 's/[^0-9]//g' > tmp.txt
if [ "`cat tmp.txt`" -ge "14" ]; then
	PASSBB="Pass-minlen"
else
	PASSBB="Fail-minlen"
fi
...
do
grep $i /etc/security/pwquality.conf | sed -e 's/[^0-9]//g' > tmp2.txt
if [ "`cat tmp2.txt`" -ge "2" ]; then
	PASSCC="Fail"
else
	PASSCC="Pass"
fi
...
{
grep "remember" /etc/pam.d/system-auth | sed -e 's/[^0-9]//g' > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
        echo -e "6.3.3,Pam Max Reuse,=,Fail" >> $REPFILE
else
	if [ "`cat tmp.txt`" -ge "10" ]; then
...
{
grep pam_wheel.so /etc/pam.d/su | grep required > tmp.txt
if [ "`cat tmp.txt`" != "auth required pam_wheel.so use_uid" ]; then
        echo -e "6.3.4,Pam Access su,=,Fail" >> $REPFILE
fi
grep wheel /etc/group | awk -F: '{print $4}' > tmp2.txt
if [ "`cat tmp2.txt`" == "" ]; then
...
{
grep PASS_MAX_DAYS /etc/login.defs | grep -v "#" | sed -e 's/[^0-9]//g' > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
        echo -e "7.1.1,Max Days Passwd,=,Fail-NOT Relevant using LDAP" >> $REPFILE
else
        if [ "`cat tmp.txt`" -ge "91" ]; then
                echo -e "7.1.1,Max Days Passwd,=,Fail-NOT Relevant using LDAP" >> $REPFILE
        else
...
{
grep PASS_MIN_DAYS /etc/login.defs | grep -v "#" | sed -e 's/[^0-9]//g' > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
        echo -e "7.1.2,Min Days Passwd,=,Fail-NOT Relevant using LDAP" >> $REPFILE
else
        if [ "`cat tmp.txt`" -le "6" ]; then
                echo -e "7.1.2,Min Days Passwd,=,Fail-NOT Relevant using LDAP" >> $REPFILE
        else
...
{
grep PASS_WARN_AGE /etc/login.defs | grep -v "#" | sed -e 's/[^0-9]//g' > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
        echo -e "7.1.3,Min Days Warn,=,Fail-NOT Relevant using LDAP" >> $REPFILE
else
        if [ "`cat tmp.txt`" -ge "2" ]; then
...
{
egrep -v "^\+" /etc/passwd | awk -F: '($1!="root" && $1!="sync" && $1!="shutdown" && $1!="halt" && $3<1000 && $7!="/sbin/nologin") {print}' > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function root_grp
{
grep "^root:" /etc/passwd | cut -f4 -d: > tmp.txt
if [ "`cat tmp.txt`" == "0" ]; then
...
{
grep "^umask 077" /etc/bashrc > tmp.txt
grep "^umask 077" /etc/profile.d/* > tmp2.txt
if [ "`cat tmp.txt`" == "" ]; then
	if [ "`cat tmp2.txt`" == "" ]; then
...
do
stat -L -c "%a %u %g" $i | egrep "644 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
	PASSDD="Fail"
else
	PASSDD="Pass"
...
{
stat -L -c "%a %u %g" /etc/passwd | egrep "644 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/shadow | egrep "0 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
stat -L -c "%a %u %g" /etc/gshadow | egrep "0 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function group_perm
{
stat -L -c "%a %u %g" /etc/group | egrep "644 0 0" > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
df --local -P | awk {'if (NR!=1) print $6'} | xargs -I '{}' find '{}' -xdev -nouser > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
df --local -P | awk {'if (NR!=1) print $6'} | xargs -I '{}' find '{}' -xdev -nogroup > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
{
/bin/cat /etc/shadow | /bin/awk -F: '($2 == "" ) { print $1 " does not have a password "}' > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
function root_zero
{
/bin/cat /etc/passwd | /bin/awk -F: '($3 == 0) { print $1 }' | grep -v root > tmp.txt
if [ "`cat tmp.txt`" == "" ]; then
...
if [ "`echo $PATH | grep :: `" != "" ]; then
  echo "Empty Directory in PATH (::)" >> tmp.txt
fi

if [ "`echo $PATH | grep :$`" != "" ]; then
  echo "Trailing : in PATH" >> tmp.txt
fi
...
while [ "$1" != "" ]; do
  ...
  if [ -d $1 ]; then
    ...
    if [ `echo $dirperm | /bin/cut -c6 ` != "-" ]; then
      echo "Group Write permission set on directory $1" >> tmp.txt
    fi
    if [ `echo $dirperm | /bin/cut -c9 ` != "-" ]; then
      echo "Other Write permission set on directory $1" >> tmp.txt
    fi
      dirown=`ls -ldH $1 | awk '{print $3}'`
      if [ "$dirown" != "root" ] ; then
        echo $1 is not owned by root >> tmp.txt
      fi
  else
    echo $1 is not a directory >> tmp.txt
  fi
  shift
done
if [ "`cat tmp.txt`" == "" ]; then
...
for dir in `/bin/cat /etc/passwd | /bin/egrep -v '(root|halt|sync|shutdown)' |\
  /bin/awk -F: '($8 == "PS" && $7 != "/sbin/nologin") { print $6 }'`; do
    ...
    if [ `echo $dirperm | /bin/cut -c6 ` != "-" ]; then
      echo "Group Write permission set on directory $dir" >> tmp.txt
    fi
    if [ `echo $dirperm | /bin/cut -c8 ` != "-" ]; then
      echo "Other Read permission set on directory $dir" >> tmp.txt
    fi
    if [ `echo $dirperm | /bin/cut -c9 ` != "-" ]; then
      echo "Other Write permission set on directory $dir" >> tmp.txt
    fi
    if [ `echo $dirperm | /bin/cut -c10 ` != "-" ]; then
      echo "Other Execute permission set on directory $dir" >> tmp.txt
    fi
done
if [ "`cat tmp.txt`" == "" ]; then
	...
	for file in $dir/.[A-Za-z0-9]*; do
		if [ ! -h "$file" -a -f "$file" ]; then
			...
			if [ `echo $fileperm | /bin/cut -c6 ` != "-" ]; then
				echo "Group Write permission set on file $file" > tmp.txt
			fi
			if [ `echo $fileperm | /bin/cut -c9 ` != "-" ]; then
				echo "Other Write permission set on file $file" >> tmp.txt
			fi
		fi
	done
done
  ...
  for file in $dir/.netrc; do
    if [ ! -h "$file" -a -f "$file" ]; then
      ...
      if [ `echo $fileperm | /bin/cut -c5 ` != "-" ]; then
        echo "Group Read set on $file" >> tmp.txt
      fi
      if [ `echo $fileperm | /bin/cut -c6 ` != "-" ]; then
        echo "Group Write set on $file" >> tmp.txt
      fi
      if [ `echo $fileperm | /bin/cut -c7 ` != "-" ]; then
        echo "Group Execute set on $file" >> tmp.txt
      fi
      if [ `echo $fileperm | /bin/cut -c8 ` != "-" ]; then
        echo "Other Read  set on $file" >> tmp.txt
      fi
      if [ `echo $fileperm | /bin/cut -c9 ` != "-" ]; then
        echo "Other Write set on $file" >> tmp.txt
      fi
      if [ `echo $fileperm | /bin/cut -c10 ` != "-" ]; then
        echo "Other Execute set on $file" >> tmp.txt
      fi
    fi
  done
done
...
for dir in `/bin/cat /etc/passwd | /bin/egrep -v '(root|halt|sync|shutdown)' |\
  ...
  for file in $dir/.rhosts; do
    if [ ! -h "$file" -a -f "$file" ]; then
      echo ".rhosts file in $dir" >> tmp.txt
    fi
  done
done
if [ "`cat tmp.txt`" == "" ]; then
...
for i in $(cut -s -d: -f4 /etc/passwd | sort -u ); do
  ...
  if [ $? -ne 0 ]; then
    echo "Group $i is referenced by /etc/passwd but does not exist in /etc/group" >> tmp.txt
  fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
cat /etc/passwd | awk -F: '{ print $1 " " $3 " " $6 }' | while read user uid dir; do
  ...
  then
    echo "The home directory ($dir) of user $user does not exist." >> tmp.txt
  fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
if [ $uid -ge 1000 -a -d "$dir" -a $user != "nfsnobody" ]; then
  ...
  if [ "$owner" != "$user" ]; then
    echo "The home directory ($dir) of user $user is owned by $owner." >> tmp.txt
  fi
fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
echo "The Output for the Audit of Control 9.2.15 - Check for Duplicate UIDs is" /bin/cat /etc/passwd | /bin/cut -f3 -d":" | /bin/sort -n | /usr/bin/uniq -c |\
  ...
  if [ $1 -gt 1 ]; then
    ...
    echo "Duplicate UID ($2): ${users}" >> tmp.txt
  fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
{
echo "" > tmp.txt
echo "# The Output for the Audit of Control 9.2.16 - Check for Duplicate GIDs is" > tmp.txt
/bin/cat /etc/group | /bin/cut -f3 -d":" | /bin/sort -n | /usr/bin/uniq -c |\
  ...
  if [ $1 -gt 1 ]; then
    grps=`/bin/gawk -F: '($3 == n) { print $1 }' n=$2 \
      /etc/group | xargs`
    echo "Duplicate GID ($2): ${grps}" >> tmp.txt
  fi
done
if [ "`cat tmp.txt | grep -v '#'`" == "" ]; then
...
/bin/cat /etc/passwd |\
  ...
  while read user uid; do
    ...
    if [ $found -eq 0 ]; then
      echo "User $user has a reserved UID ($uid)." > tmp.txt
      if [ "`cat tmp.txt | sed -e 's/[(,),.]//g' | awk '{print $7}'`" -ge "500" ]; then
        > tmp.txt
      else
        echo "User $user has a reserved UID ($uid)." >> tmp2.txt
        > tmp.txt
      fi
    fi
  done
...
echo "The Output for the Audit of Control 9.2.18 - Check for Duplicate User Names is" cat /etc/passwd | cut -f1 -d":" | /bin/sort -n | /usr/bin/uniq -c |\
  ...
  if [ $1 -gt 1 ]; then
    uids=`/bin/gawk -F: '($1 == n) { print $3 }' n=$2 \
      /etc/passwd | xargs`
    echo "Duplicate User Name ($2): ${uids}" > tmp.txt
  fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
echo "The Output for the Audit of Control 9.2.19 - Check for Duplicate Group Names is" cat /etc/group | cut -f1 -d":" | /bin/sort -n | /usr/bin/uniq -c |\
  ...
  if [ $1 -gt 1 ]; then
    ...
    /etc/group | xargs`
    echo "Duplicate Group Name ($2): ${gids}" > tmp.txt
  fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
for dir in `/bin/cat /etc/passwd |\
  ...
  if [ ! -h "$dir/.netrc" -a -f "$dir/.netrc" ]; then
    echo ".netrc file $dir/.netrc exists" > tmp.txt
  fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
/bin/awk -F: '{ print $6 }'`; do
  if [ ! -h "$dir/.forward" -a -f "$dir/.forward" ]; then
    echo ".forward file $dir/.forward exists" > tmp.txt
  fi
done
if [ "`cat tmp.txt`" == "" ]; then
...
###CGLG 14-Oct-2017     Revision-1.0.1-05- General Clean Up for github
###CGLG 
###CGLG 
